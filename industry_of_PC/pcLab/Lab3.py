...
from keras.layers import Input, Conv2D, MaxPool2D, UpSampling2D, Flatten, Activation
from keras.models import Model
Datadir = os.getcwd() + "/Fabric"
Categories = ["overkills/edges"]
...
def AE_sample(input_shape = (64, 64, 1)):
...
for i in range(n):
    ...
    plt.imshow(result[i].reshape(64, 64))
    plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.show()
