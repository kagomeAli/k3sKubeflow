...
from keras.layers import Input, Dense, Activation, Conv2D, MaxPool2D, Dense, Flatten
...
def CNN_sample(input_shape = (64, 64, 1)):
   start = Input(input_shape)
   c1_1 = Conv2D(8, (4, 4), activation='relu' ,padding='same')(start)
   c1_2 = MaxPool2D((4, 4), padding='same')(c1_1)
   print(Model(start, c1_2).layers[2].output)
   F1 = Flatten()(c1_2)
...
training_process = CNN.fit(x_train, y_train,
             batch_size=64,
             epochs=30,
             verbose=1,
