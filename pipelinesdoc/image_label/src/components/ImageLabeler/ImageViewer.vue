<template>
  <div class="position-relative">
    ...
    <div class="position-absolute cover">
      <div v-for="(item, index) in image.labels" :key="index"
        class="position-absolute label-box"
        :style="{
          'top': (item.ty * heightZoom) + 'px',
          'left': ((item.lx * widthZoom) + widthOffset) + 'px',
          'height': ((item.by - item.ty) * heightZoom) + 'px',
          'width': ((item.rx - item.lx) * widthZoom) + 'px',
        }">
      </div>
    </div>
    ...
    renderImage: function renderImage() {
      if (this.image.imageReader === null || this.image.imageReader.result === null) {
        this.worker.postMessage('loadImage', [this.image.htmlFileObject])
          .then((response) => {
            this.$store.commit('setImageContent', response);
            this.$nextTick(() => {
